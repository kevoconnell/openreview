...
import { syncCheckedInViewerAssets } from "./sync-viewer-assets.js";
import { syncViewerPayloads } from "./payload.js";
...
async function resolveControlServerRunner({
  ...
  const distJsRunnerPath = path.resolve(
    moduleDir,
    "../../dist/viewer/run-control-server.js",
  );
...
export async function runViewerControlServer({
  ...
  const server = http.createServer(async (request, response) => {
    ...
    if (request.method === "POST" && url.pathname === "/control/reexamine") {
      ...
      void (async () => {
        try {
          await generateReview({ repoPath, mode: "incremental" });
          await refreshViewer();
          reexamineState = "idle";
          lastError = null;
        } catch (error) {
