...
import { ensureViewerBundle } from "./bundle.js";
...
async function readViewerAppJs(): Promise<string> {
  for (const candidate of getViewerAssetDirCandidates()) {
    for (const fileName of ["viewer-app.js", "viewer-app.bundle.js"]) {
      const viewerAppJsPath = path.join(candidate, fileName);
  ...
  <head>
    ...
    <style>
      ...
      * {
      ...
      @keyframes git-review-status-pulse {
        0%,
        100% {
          transform: scale(0.9);
          opacity: 0.6;
      ...
      @keyframes git-review-progress-scan {
        0% {
      ...
      .git-review-status-dot {
        width: 8px;
        height: 8px;
        border-radius: 999px;
        background: var(--accent-2);
        box-shadow: 0 0 0 6px rgba(56, 189, 248, 0.14);
        animation: git-review-status-pulse 1.35s ease-in-out infinite;
      }
      ...
      .git-review-progress-bar {
        ...
        background: linear-gradient(
          ...
          var(--accent-2) 50%,
          rgba(125, 211, 252, 0.7) 72%,
          transparent 100%
        );
        animation: git-review-progress-scan 1.5s ease-in-out infinite;
      }

      .git-review-reexamine-message {
        margin: 10px 18px 0;
      }
